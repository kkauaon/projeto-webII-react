...
function Register() {
    ...
    const onSubmit = data => {
		setLoading(true);
        api.post('/users/register', data).then(user => {
			setLoading(false);
			console.log('Registro realizado com sucesso!!! Bem-vindo ' + user.data.username);
			navigate('/');
		}).catch(err => {
			setLoading(false);
			setError(err.response?.data?.error || 'Ocorreu um erro ao realizar o registro.');
			console.error(err);
		});
    };
            ...
            <div>
                ...
                >
                    <Form className="loginFormContainer">
						...
						<button className='createAccountButton' onClick={() => navigate('/login')}>
							Entrar
						</button>
                    </Form>
                </Formik>
